...
public class LoadBalancerFactory {
     ...
     * @param serviceId Yaratılacak Load Balancer için hedef servisin adı
     * @return Belirtilen stratejiye uygun bir şekilde yaratılmış ReactorLoadBalancer nesnesi
     */
    public ReactorLoadBalancer<ServiceInstance> createLoadBalancer(LoadBalancerStrategy strategy, LoadBalancerClientFactory factory, String serviceId) {
        final var type = ServiceInstanceListSupplier.class;

        return switch (strategy) {
            case RANDOM -> new RandomLoadBalancer(factory.getLazyProvider(serviceId, type), strategy.name());
            case ROUND_ROBIN -> new RoundRobinLoadBalancer(factory.getLazyProvider(serviceId, type), serviceId);
            case LEAST_CONNECTION -> throw new UnsupportedOperationException("Strategy not implemented yet");
        };
    }
}
